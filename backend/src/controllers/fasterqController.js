...
const parseCall = (raw) => {
    ...
    return {
        fasterqId:    (raw.id || raw.call_id || raw.uuid || null)?.toString() || null,
        direction,
...
// ═══════════════════════════════════════════════════════════════════════════
// POST /api/fasterq/webhook
// ═══════════════════════════════════════════════════════════════════════════
const fasterqWebhook = async (req, res, next) => {
    try {
        ...
        for (const raw of payloads) {
            ...
            if (call.fasterqId) {
                const exists = await prisma.fasterqCall.findUnique({
                    where: { fasterqId: call.fasterqId },
                });
                if (exists) {
                    results.push({ fasterqId: call.fasterqId, saved: false, reason: "duplicate" });
                    continue;
                }
            }
            ...
            const saved = await prisma.fasterqCall.create({
            ...
            if (lead) {
                ...
                await prisma.activity.create({
                    data: {
                        leadId: lead.id,
                        action: `Fasterq ${call.direction} call — ${call.status} · ${mins}m ${secs}s${call.agentName ? ` by ${call.agentName}` : ""}`,
                    },
                });
            }

            results.push({ fasterqId: call.fasterqId, saved: saved.id, leadMatched: !!lead });
        }
...
const getCalls = async (req, res, next) => {
    try {
        ...
        const [calls, total] = await Promise.all([
            ...
            prisma.fasterqCall.count({ where }),
        ]);
...
module.exports = { fasterqWebhook, getCalls, getStats };
